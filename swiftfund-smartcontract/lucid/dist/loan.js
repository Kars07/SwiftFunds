import { Blockfrost, Data, Lucid, validatorToAddress, getAddressDetails } from "@lucid-evolution/lucid";
...
// const BorrowerPrivateKey = "6741ad70e44567a612158287d607f850317af90643d64afe8d10da10"
// lucid.selectWallet.fromPrivateKey(BorrowerPrivateKey);
// console.log("Borrower Wallet selected");
// const BorrowerSeedPhrase = "derive indicate hold frost light random never demise bonus plunge choose junk harvest elbow nerve act must civil clerk situate ethics nation auction drink"
// lucid.selectWallet.fromSeed(BorrowerSeedPhrase);
// console.log("Borrower Wallet selected")
const loanRequestValidatorScript = {
    type: "PlutusV2",
...
const FundRequestValidatorScript = {
    type: "PlutusV2",
...
const RepayRequestValidatorScript = {
    type: "PlutusV2",
...
const LoanRequestAddress = validatorToAddress("Preprod", loanRequestValidatorScript);
const FundLoanAddress = validatorToAddress("Preprod", FundRequestValidatorScript);
const RepayLoanAddress = validatorToAddress("Preprod", RepayRequestValidatorScript);
const BorrowerDetails = getAddressDetails("addr_test1qpn5rttsu3zk0fsjzkpg04s8lpgrz7heqepavjh735gd5yp0h9u692rgepwzh7ys20akzrcp4prp8gtwhssgnpxnueesnuh85k");
const BorrowerPKH = BorrowerDetails.paymentCredential?.hash;
const BorrowerAddress = BorrowerDetails.address.bech32;
const LenderDetails = getAddressDetails("addr_test1qr6kkw3lmmgr55wxdd4zrpnnjxwqzcv8xlae8q95h9gg3nnr62p4sn24mqx08pef7sr0zyksp76u0d6cyh9gu0v963wqvh7gsx");
const LenderPKH = LenderDetails.paymentCredential?.hash;
const LenderAddress = LenderDetails.address.bech32;
...
const deadline = new Date(Date.now() + 1000 * 60 * 60 * 24 * 7);
...
const BorrowerDatum = loanRequestSchema;
const datum = {
...
const redeemerType = fundloanredeemerschema;
const FundRedeemer = {
    lenderPKH: LenderPKH,
    loanAmount: loanAmount
};
const dtm = Data.to(datum, BorrowerDatum);
const fundredeem = Data.to(FundRedeemer, redeemerType);
async function requestLoan(lucid, borrowerPKH, loanAmount, interest, deadline) {
...
async function fundLoan(lucid, lenderPKH, loanAmount) {
    ...
    while (retries > 0) {
        ...
        const utxosAtScript = await lucid.utxosAt(LoanRequestAddress);
        console.log("UTxOs at script address:", utxosAtScript);
        const borrowerUTxO = utxosAtScript.filter((utxo) => {
            if (!utxo.datum) {
            ...
            try {
                ...
                return (onChainDatum.borrowerPKH === datum.borrowerPKH &&
